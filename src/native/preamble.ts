...
import { CELL_SEGMENTER_SHIM } from "./cell-segmenter-shim";
...
const PREAMBLE = [
  ...
  "",
  '// bun\'s fetch takes compress: "gzip" | { encoding: "gzip", level } and encodes the request body;',
  "// node's fetch ignores the option. installed before the net-intercept anchor below so wrappers",
  "// placed there capture this fetch and still see the plain body.",
  "const __cccGzip = promisify(gzip);",
  "const __cccUncompressedFetch = globalThis.fetch;",
  "globalThis.fetch = async function fetch(input, init) {",
  "  if (init?.compress == null) return __cccUncompressedFetch(input, init);",
  "  const { compress, ...rest } = init;",
  '  const encoding = typeof compress === "string" ? compress : compress.encoding;',
  '  if (encoding !== "gzip") throw new TypeError(`fetch compress: unsupported encoding ${encoding}`);',
  "  const { body } = rest;",
  '  if (typeof body !== "string" && !(body instanceof Uint8Array) && !(body instanceof ArrayBuffer)) {',
  '    throw new TypeError("fetch compress: body must be a string, Uint8Array, or ArrayBuffer");',
  "  }",
  '  const headers = new Headers(rest.headers);',
  '  headers.set("content-encoding", "gzip");',
  '  headers.delete("content-length");',
  '  const level = typeof compress === "object" ? compress.level : undefined;',
  "  return __cccUncompressedFetch(input, { ...rest, headers, body: await __cccGzip(Buffer.from(body), level === undefined ? {} : { level }) });",
  "};",
  "",
  ...
  "// string-width measures printable ASCII as one column per character; skip its per-grapheme segmentation",
  "const __cccPrintableAsciiRe = /^[\\x20-\\x7E]*$/;",
  "function __cccStringWidth(value) {",
  "  const text = String(value);",
  "  if (__cccPrintableAsciiRe.test(text)) return text.length;",
  '  if (typeof __cccStringWidthPackage === "function") return __cccStringWidthPackage(text);',
  "  let width = 0;",
  ...
  "  return width;",
  "}",
  "// 2.1.274 slices wrapped transcript lines and viewport rows with Bun.sliceAnsi(text, start, end):",
  "// visible-column bounds, escape sequences kept, SGR styles active before `start` re-opened",
  "// so a slice renders like the original. every caller shrinks `end` while the width overflows.",
  "function __cccSliceAnsi(value, start, end) {",
  "  const text = String(value);",
  "  const from = Math.max(0, Number(start) || 0);",
  "  const to = end === undefined ? Infinity : Math.max(from, Number(end) || 0);",
  "  const escapeRe = /\\x1B(?:[@-Z\\\\-_]|\\[[0-?]*[ -/]*[@-~]|\\][^\\x07]*(?:\\x07|\\x1B\\\\))/y;",
  ...
  "    escapeRe.lastIndex = index;",
  "    const escapeMatch = escapeRe.exec(text);",
  "    if (escapeMatch) {",
  "      const sequence = escapeMatch[0];",
  "      index += sequence.length;",
  "      if (sgrRe.test(sequence)) {",
  '        if (sequence === "\\x1B[0m" || sequence === "\\x1B[m") styles.length = 0;',
  "        else styles.push(sequence);",
  "      }",
  "      if (emitted && column < to) out += sequence;",
  "      continue;",
  "    }",
  "    const rest = text.slice(index);",
  "    let grapheme;",
  "    if (graphemes) {",
  "      const iterator = graphemes.segment(rest)[Symbol.iterator]().next();",
  "      grapheme = iterator.done ? rest : iterator.value.segment;",
  "    } else {",
  "      const code = rest.codePointAt(0) ?? 0;",
  "      grapheme = rest.slice(0, code > 0xffff ? 2 : 1);",
  "    }",
  "    const width = __cccStringWidth(grapheme);",
  "    const next = column + width;",
  "    if (column >= from && next <= to) {",
  "      if (!emitted) { out += styles.join(\"\"); emitted = true; }",
  "      out += grapheme;",
  "    } else if (next > to) {",
  "      break;",
  "    }",
  ...
  "  }",
  '  if (emitted && styles.length > 0 && !out.endsWith("\\x1B[0m")) out += "\\x1B[0m";',
  "  return out;",
  "}",
  "function __cccSleepSync(ms) {",
  ...
  "  sliceAnsi: __cccSliceAnsi,",
  "  sleepSync: __cccSleepSync,",
  "  Transpiler: __cccTranspiler,",
  '  get Image() { return __baseRequire("sharp"); },',
  ...
  "  // the bundle guards only `typeof Bun` before `Bun.unsafe.setJITPolicy?.()`; node has no JIT policy knob",
  "  unsafe: {},",
...
const GRAPH_PREAMBLE_EXTRAS = [
  ...
  "};",
  "// the 2.1.274 ink renderer throws at startup without ant.CellSegmenter",
  CELL_SEGMENTER_SHIM,
  "__cccBun.TOML = {",
  "  parse(text) {",
